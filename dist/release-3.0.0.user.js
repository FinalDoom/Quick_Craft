...
(function () {
    ...
    ***************************************************************************** */
    ...
    var scripts = {
    	...
    	test: "env TS_NODE_COMPILER_OPTIONS='{\"module\": \"commonjs\" }' mocha --reporter spec -r ts-node/register test/**/*.ts",
...
.recipe_buttons {
    ...
    flex-direction: column;
        ...
        async function open_crafting_submenu(recipe, purchasable) {
            ...
            const createCraftingActions = (available) => {
                ...
                return $('<div>')
                    ...
                    .append((craftNumberSelect = $('<select>').append(...Array(currentCraft.available)
                    .fill(undefined)
                    .map((_, i) => `<option value="${i + 1}">${i + 1}</option>`))))
                    .append($('<button>Craft</button>').click(doCraft))
                    .append($('<button class="quick_craft_button">Craft maximum</button>').click(function () {
            ...
            const createIngredientLine = (ingredient, maxWithPurchase) => {
                ...
                return $('<div>')
                    ...
                    .append($('<a class="quick_craft_button">$</a>')
                    ...
                    cursor: 'pointer',
                    padding: '0 .25rem',
                }), `${titleCaseFromUnderscored(ingredName)}:`, `<div style="display: inline-flex;" class="ingredient_quantity"><span>${qtyOnHand}</span><span>/</span><span>${qtyPerCraft}</span></div>`, maxWithPurchase > qtyOnHand / qtyPerCraft
                    ...
                    else if (purchasable.length < currentCraft.ingredients.length - 1)
                        purchasable.push(ingredName);
                    close_crafting_submenu();
                    open_crafting_submenu(recipe, purchasable);
                });
            };
            const maxWithPurchase = purchasable.length
                ? Math.min(...currentCraft.ingredients.map((ingredient) => purchasable.includes(ingredient.name)
                    ? Number.MAX_SAFE_INTEGER
                    : Math.floor(ingredient.onHand / ingredient.qty)))
                : currentCraft.available;
            $('#current_craft_box').append((craftingSubmenu = $('<div id="crafting-submenu">')
                ...
                .append(INVENTORY.itemCount(String(recipe.itemId)) > 0
                ...
                .append(currentCraft.available !== maxWithPurchase
                ? $(`<span>`)
                ...
                : '', $('<a>')
                .text('?')
                .attr('title', 'Click ingredients to mark as purchasable and calculate +purchase needed and max possible crafted.')
                .wrap('<sup>')
                .parent()))
        ...
        //
        const createRecipeButton = (recipe) => {
            const name = recipe.name || ingredients[recipe.itemId].name;
            return $(`<button class="recipe_button-book_${recipe.book.replace(/ /g, '_')}" id="recipe_button-${name.replace(/ /g, '_')}">${name}</button>`)
                .css({
        ...
        $('#crafting_recipes').before('<div style="clear: both; margin-bottom: 1rem;">', $('<div id="quick-crafter">')
            .css({
            display: 'block',
            margin: '0 auto 1rem',
            ...
            .append('<div id="current_craft_box">', '<p>Having trouble? Try refreshing if it seems stuck. Turn off this script before manual crafting for a better experience.</p>', $('<button class="quick_craft_button">Clear</button>')
            .css({ marginBottom: '1rem', backgroundColor: 'red' })
            .click(() => close_crafting_submenu()), $('<div>')
            .css({ display: 'flex', flexDirection: 'row', columnGap: '.25rem', alignItems: 'center' })
            .append('<span>Click on the buttons below to show or hide crafting categories - </span>', $('<button class="quick_craft_button">Hide all</button>')
            .css({ backgroundColor: 'red' })
            .click(function () {
            BOOKS.forEach((book) => {
                if (STORE.selectedBooks.includes(book))
                    $(`#${book.replace(/ /g, '_')}`).trigger('click');
            });
            ...
            //
            .append($('<div class="recipe_buttons">').append(BOOKS.map((bookName) => $(`<div class="recipe_book_section" id="recipe_book_section-${bookName.replace(/ /g, '_')}">`)
            ...
            //
            .append(`<p style="float:right;margin-top:-20px;margin-right:5px;">Quick Crafter by <a href="/user.php?id=58819">KingKrab23</a> v<a href="https://github.com/KingKrab23/Quick_Craft/raw/master/GGn%20Quick%20Crafting.user.js">${pkg.version}</a></p>`));
        // Persist selected recipe
        if (STORE.currentCraft)
            $(`#${STORE.currentCraft.replace(/ /g, '_')}`).trigger('click');
    })();
