...
Please disable this userscript until you have one as this prompt will continue to show until you enter one in.`);
      ...
      async function open_crafting_submenu(recipe, purchasable) {
          ...
          const createCraftingActions = (available) => {
              ...
              const doCraft = async () => {
                  ...
                  isCrafting = true;
                  Array.from(document.querySelectorAll('#crafting-submenu button, #crafting-submenu select')).forEach((elem) => {
                      elem.disabled = true;
                      elem.classList.add('disabled');
                  });
                  let craftNumber = Number(craftNumberSelect.querySelector('option:selected').value);
                  await (async () => {
          ...
          const createIngredientLine = (ingredient, maxWithPurchase) => {
              ...
              if (purchasable.includes(ingredName))
                  line.classList.add('crafting-panel-info__ingredient--purchasable');
          ...
          currentCraft.ingredients.forEach((ingredient) => ingredientsDiv.append(createIngredientLine(ingredient, maxWithPurchase)));
      ...
      const createRecipeButton = (recipe) => {
          ...
          button.classList.add('recipes__recipe', `recipes__recipe--book-${recipe.book.toLocaleLowerCase().replace(/ /g, '-')}`);
      ...
      betweenBooks.addEventListener('change', function () {
          ...
          if (checked) {
              ...
              document.querySelector('.recipe-buttons').classList.remove('recipe-buttons--extra-space');
          }
      ...
      BOOKS.forEach((name) => {
          ...
          button.classList.add('crafting-panel-filters__books-button', `crafting-panel-filters__books-button--book-${name.toLocaleLowerCase().replace(/ /g, '-')}`);
          if (STORE.selectedBooks.includes(name))
          ...
          button.addEventListener('click', function () {
              ...
              Array.from(document.querySelectorAll(`.recipe_button-book_${name.replace(/ /g, '_')}`)).forEach((elem) => (elem.disabled = disabled));
              if (disabled) {
      ...
      if (await GM.getValue('SEG', false))
          recipeButtonDiv.classList.add('recipe-buttons--extra-space');
